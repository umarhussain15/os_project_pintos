...
start_process (void *file_name_)
{
  ...
  if (!success) {
    // process failed to load so set exit code -1
    // get_process_info(thread_current()->tid)->exit_code=-1;
    // get_process_info(thread_current()->tid)->execw=1;
    // thread_current()->inf=get_process_info(thread_current()->tid);
    sema_up(&get_process_info(thread_current()->tid)->cont);
    thread_exit ();
  }
  else{
  //if process successfully created then sema_up so that
  // exec can return pid(tid_t)
  get_process_info(thread_current()->tid)->loaded=1;
  // get_process_info(thread_current()->tid)->execw=1;
  sema_up(&get_process_info(thread_current()->tid)->cont);
  }
...
{  
  ...
  if (pi == NULL){
    pi=get_process_exit(child_tid);
    if(pi==NULL)
    return -1;
  }
  
  // check for same thread calling wait again
  if(pi->waiting==1) return -1;
  
  pi->waiting=1;
  // every process has it wait sema for 
  //synch between parent and child
  sema_down(&pi->wait);
